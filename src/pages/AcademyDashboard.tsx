...
import {
  ArrowLeft, BookOpen, Trophy, Clock, Flame, Target, TrendingUp,
  Calendar, ChevronRight, Star, GraduationCap, Zap
} from "lucide-react";
      ...
      <div className="container max-w-5xl mx-auto px-4 py-8 space-y-8">
        <div className="flex items-center justify-between">
          <Button variant="ghost" size="sm" onClick={() => navigate("/academy")}>
            <ArrowLeft className="h-4 w-4 mr-2" /> Academy
          </Button>
        </div>
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-muted-foreground" /> Activité (12 semaines)
                </h3>
                {streak > 0 && (
                  <Badge className="text-[10px] bg-amber-500/10 text-amber-600 border-amber-500/20">
                    <Flame className="h-3 w-3 mr-1" /> {streak} jour{streak > 1 ? "s" : ""} consécutif{streak > 1 ? "s" : ""}
                  </Badge>
                )}
              </div>
                  ...
                  <div key={wi} className="flex flex-col gap-1">
                    {week.map(day => {
                      ...
                      return (
                        <div
                          key={day}
                          className={cn("h-3 w-3 rounded-sm transition-colors", getHeatColor(count))}
                          title={`${day}: ${count} activité${count > 1 ? "s" : ""}`}
                        />
                      );
                    })}
                  </div>
        ...
        {activeEnrollments.length > 0 && (
          <section className="space-y-3">
            <h2 className="text-lg font-display font-bold flex items-center gap-2">
              <TrendingUp className="h-5 w-5 text-primary" /> Parcours en cours
            </h2>
        ...
        {completedEnrollments.length > 0 && (
          <section className="space-y-3">
            <h2 className="text-lg font-display font-bold flex items-center gap-2">
              <Trophy className="h-5 w-5 text-amber-500" /> Parcours terminés
            </h2>
        ...
        {allProgress.filter((p: any) => p.completed_at).length > 0 && (
          <section className="space-y-3">
            <h2 className="text-lg font-display font-bold flex items-center gap-2">
              <Zap className="h-5 w-5 text-primary" /> Activité récente
            </h2>
                  ...
                  <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/30 border">
                    ...
                    {p.score != null && (
                      <Badge variant="outline" className="text-[10px]">
                        <Star className="h-2.5 w-2.5 mr-1" /> {p.score}%
                      </Badge>
                    )}
                  </div>
                ))}
        ...
        {enrollments.length === 0 && (
          <Card className="border-dashed">
            <CardContent className="p-12 text-center space-y-3">
              ...
              <Button onClick={() => navigate("/academy")}>Découvrir les parcours</Button>
            </CardContent>
          </Card>
        )}
      </div>
