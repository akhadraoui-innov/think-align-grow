...
export default function AcademyCertificates() {
  ...
  const { data: certificates = [], isLoading } = useQuery({
    ...
    queryFn: async () => { const { data } = await supabase.from("academy_certificates").select("*, academy_paths(name, description, difficulty, estimated_hours, skills, aptitudes, professional_outcomes, cover_image_url, updated_at, created_at)").eq("user_id", user!.id).order("issued_at", { ascending: false }); return data || []; },
  });
