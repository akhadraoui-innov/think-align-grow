...
export default function AcademyPath() {
  ...
  if (!path) return <PageTransition><div className="container max-w-4xl mx-auto px-4 py-8 text-center"><p className="text-muted-foreground">Parcours introuvable.</p><Button variant="ghost" className="mt-4" onClick={() => navigate("/academy")}><ArrowLeft className="h-4 w-4 mr-2" /> Retour</Button></div></PageTransition>;
    ...
    <PageTransition>
      <div className="container max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border">
          <div className="p-8 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm">
              ...
              {(path as any).academy_personae && <Badge variant="outline" className="text-xs">{(path as any).academy_personae.name}</Badge>}
            ...
            {user && !enrollment && <Button onClick={() => enrollMutation.mutate()} disabled={enrollMutation.isPending} size="lg" className="gap-2 shadow-lg"><PlayCircle className="h-5 w-5" /> S'inscrire au parcours</Button>}
            {enrollment && (
              <div className="space-y-3">
                ...
                {progressPct < 100 && firstIncomplete && <Button size="sm" onClick={() => navigate(`/academy/module/${firstIncomplete.module_id}?pathId=${id}`)} className="gap-2"><PlayCircle className="h-4 w-4" /> Continuer</Button>}
              </div>
            )}
          </div>
        </motion.div>
        ...
        <section className="space-y-4">
          ...
          {modules.length === 0 ? (
            <Card className="border-dashed"><CardContent className="p-6 text-center text-muted-foreground text-sm">Aucun module dans ce parcours pour le moment.</CardContent></Card>
          ) : (
              ...
              {modules.map((pm: any, idx: number) => {
                ...
                return (
                  <motion.div key={pm.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} className="relative pl-14 pb-6">
                    ...
                    <Card className={cn("transition-all overflow-hidden", isLocked && "opacity-50", !isLocked && "hover:shadow-md cursor-pointer")} onClick={() => !isLocked && navigate(`/academy/module/${pm.module_id}?pathId=${id}`)}>
                        ...
                        {objectives.length > 0 && !isLocked && (
                          <div className="flex flex-wrap gap-1.5 pl-14">
                            {objectives.slice(0, 3).map((obj, i) => (
                              <span key={i} className="text-[10px] text-muted-foreground bg-muted/50 px-2 py-0.5 rounded-full">{obj}</span>
                            ))}
                          </div>
                        )}
                      </CardContent>
