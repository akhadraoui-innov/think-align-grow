...
export default function AdminAcademyPaths() {
  ...
  const generateAI = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setAiOpen(false);
      if (data.path_id) navigate(`/portal/academie/paths/${data.path_id}`);
    },
    onError: (e: any) => toast.error(e.message),
  });
              ...
              <Table>
                ...
                <TableBody>
                  {filtered.map((p: any) => {
                    const diff = difficultyConfig[p.difficulty || "intermediate"] || difficultyConfig.intermediate;
                    return (
                      <TableRow key={p.id} className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/portal/academie/paths/${p.id}`)}>
                        <TableCell>
