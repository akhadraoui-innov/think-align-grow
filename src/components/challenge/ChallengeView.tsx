...
export function ChallengeView({ template, workshopId, cards, pillars, isHost, readOnly }: ChallengeViewProps) {
  ...
  const stagedCardIds = useMemo(() => new Set(stagingItems.map(i => i.card_id)), [stagingItems]);
    ...
    <div className="flex-1 flex flex-row min-h-0">
      ...
      {!readOnly && (
        <CardSidebar
          cards={cards}
          pillars={pillars}
          onAddCard={handleAddFromSidebar}
          isMobile={isMobile}
        />
      )}
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <AnimatePresence mode="wait">
            {currentSubject && (
              viewMode === "list" ? (
                <motion.div key={`list-${currentSubject.id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full">
                  <SubjectCanvas {...canvasProps} />
                </motion.div>
              ) : (
                <motion.div key={`board-${currentSubject.id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full">
                  <ChallengeBoard {...canvasProps} />
                </motion.div>
              )
            )}
          </AnimatePresence>
