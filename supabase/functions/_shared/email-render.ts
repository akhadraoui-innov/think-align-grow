// Markdown → HTML renderer with co-branded GROWTHINNOV layout
// Lightweight: no external deps, handles the markdown subset used in templates.
...
export interface RenderInput {
  ...
  branding: {
    ...
    orgName?: string | null;
    coBrand: boolean; // show org logo alongside GROWTHINNOV
  };
  footer?: {
...
function renderBody(markdown: string): string {
  ...
  for (const raw of lines) {
    ...
    // Button shortcode: {{button:Label|https://url}}
...
// ─── Layout shell ──────────────────────────────────────────────────────────
function header(branding: RenderInput["branding"]): string {
...
function footer(branding: RenderInput["branding"], footerOpts: RenderInput["footer"]): string {
